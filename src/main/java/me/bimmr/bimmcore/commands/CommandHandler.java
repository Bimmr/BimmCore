...
public class CommandHandler implements TabCompleter, CommandExecutor {
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
		...
		if (args.length == 1) {
			...
			for (SubCommand subCommand : this.commands)
				if (subCommand.getName().equalsIgnoreCase(args[0]) || subCommand.getAliases().contains(args[0].toLowerCase())) params = subCommand.getTabs(args);
			if (params != null && !params.isEmpty()) return TabCompletionHelper.getPossibleCompletionsForGivenArgs(args, params);
			else return TabCompletionHelper.getPossibleCompletionsForGivenArgs(args, TabCompletionHelper.getOnlinePlayers());
		}
	}
